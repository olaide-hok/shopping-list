...
export default function ProductScreen() {
    ...
    React.useEffect(() => {
        if (name === undefined) {
            router.replace(`/list/${listId}/index`);
        }
    }, [listId, name]);
...
function ProductContent({
    ...
    const [name, setName] = useShoppingListProductCell(
        listId,
        productId,
        'name'
    );
    ...
    const createdBy = useShoppingListProductCreatedByNickname(
        listId,
        productId
    );
    ...
    return (
        <BodyScrollView
            ...
            <FieldItem
                label="Created at"
                value={
                    createdAt ? new Date(createdAt).toDateString() : 'unknown'
                }
            />
            <FieldItem
                label="Quantity"
            ...
            <View style={{flexDirection: 'row', flexWrap: 'wrap'}}>
                {userNicknames.map((nickname, index) => (
                    <ThemedText key={nickname + index} type="default">
                        {nickname}
